...
const HeroSection = () => {
  ...
  useEffect(() => {
    const img = new Image();
    img.src = "/fundo-hero.jpg"; // ou '/fundo-hero.jpg' se for o caso
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      {/* Background only appears after loaded */}
      <div
        className={`absolute inset-0 bg-center bg-no-repeat transition-opacity duration-500 ${
          bgLoaded ? "opacity-100 bg-cover" : "opacity-0"
        }`}
        style={{
          backgroundImage: bgLoaded ? `url('/fundo-hero.jpg')` : "none",
          backgroundColor: "#0f0f0f", // cor fallback
        }}
      >
        {bgLoaded && (
